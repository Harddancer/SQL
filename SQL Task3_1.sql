...
ALTER TABLE `profiles` ADD CONSTRAINT fk_photo_id
   ...
   CREATE TABLE hobby(
    id SERIAL PRIMARY KEY,
   	type_hobby VARCHAR(100) NOT NULL COMMENT 'нОВЫЙ КОММЕНТ',
   	progress VARCHAR(100) NOT NULL,
   	period_from DATE
   	);
  -- 2 я Новая таблица posts 1-М users к posts
  ...
  -- Новая таблица для связки М-М hobby и users 
  DROP TABLE IF EXISTS hobby_users;
  CREATE TABLE hobby_users (
    PRIMARY KEY (hobby_id, user_id),
  	hobby_id BIGINT unsigned NOT NULL,
    user_id BIGINT unsigned NOT NULL,
    FOREIGN KEY (hobby_id) REFERENCES hobby(id) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE
);

-- Новая таблица для связки 1-1 users_adress
-- 1-1 это когда один  первичный ключ, который является сразу и внешним, ссылается на первичный ключ другой таблицы
  DROP TABLE IF EXISTS users_adress;
  CREATE TABLE users_adress(
  id SERIAL PRIMARY KEY,
  adress VARCHAR(30),
  -- adress_id BIGINT UNSIGNED UNIQUE COMMENT 'Атрибуты ключа должны соотвестовать тарибутам ключа второй таблицы',
